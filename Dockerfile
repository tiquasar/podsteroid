# ─────────────────────────────────────────────────────────────────────────────
# PodSteroid Unified Dockerfile
# Combines Custom Kernel, Initramfs (Alpine VM) and QEMU (Android ARM64) builds.
# ─────────────────────────────────────────────────────────────────────────────

# Global build arg: the rootfs stage must be built for aarch64 (the guest is
# arm64), even when the build host is x86_64. A variable (not a hardcoded
# constant) keeps the FROM --platform lint check happy.
ARG ROOTFS_PLATFORM=linux/arm64/v8
...
COPY podsteroid_kernel.config /tmp/podsteroid_kernel.config
...
RUN cd linux-${KERNEL_VERSION} \
    ...
    && ./scripts/kconfig/merge_config.sh -m .config /tmp/podsteroid_kernel.config \
    ...
    && echo "=== verifying netfilter match/reject options (y or m accepted) ===" \
    && for opt in NETFILTER_XT_MATCH_MULTIPORT NETFILTER_XT_MATCH_IPRANGE NETFILTER_XT_MATCH_OWNER \
    NETFILTER_XT_TARGET_REJECT IP_NF_TARGET_REJECT IP6_NF_TARGET_REJECT \
    NF_REJECT_IPV4 NF_REJECT_IPV6 NFT_REJECT; do \
    grep -qE "^CONFIG_${opt}=(y|m)\$" .config \
    || { echo "WARNING: CONFIG_${opt} is neither y nor m (netfilter feature unavailable)" >&2; \
    grep "CONFIG_${opt}" .config >&2 || true; }; \
    done \
...
# Keep only modules init-podsteroid actually uses; everything else (DSA, pinctrl,
# mediatek, renesas, hardware-specific drivers) is dead weight in a VM.
# init-podsteroid runs `depmod -a` at boot so we don't need to regenerate modules.dep here.
...
FROM --platform=$ROOTFS_PLATFORM alpine:3.24 AS rootfs-builder
...
# (apk exits 255). Re-running update+add a few times rides out the transient.
RUN set -e; \
    ok=0; \
    for i in 1 2 3 4 5; do \
        if apk update && apk add --no-cache \
            bash busybox busybox-extras ttyd podman \
            netavark aardvark-dns fuse-overlayfs slirp4netns iptables \
            shadow-subids ca-certificates crun curl e2fsprogs util-linux openrc \
            dropbear ncurses-terminfo-base musl-locales kmod fastfetch; then \
            ok=1; break; \
        fi; \
        echo "apk install attempt $i failed, retrying in 5s..."; sleep 5; \
    done; \
    if [ "$ok" -ne 1 ]; then echo "apk install failed after retries"; exit 1; fi
COPY init-podsteroid /init
RUN chmod +x /init
...
RUN printf '#ifndef PODSTEROID_ICONV_H\n#define PODSTEROID_ICONV_H\n#include <stddef.h>\ntypedef void *iconv_t;\niconv_t iconv_open(const char *tocode, const char *fromcode);\nsize_t iconv(iconv_t cd, char **inbuf, size_t *inbytesleft, char **outbuf, size_t *outbytesleft);\nint iconv_close(iconv_t cd);\n#endif\n' > ${PREFIX}/include/iconv.h \
...
RUN (wget -q --tries=3 https://download.savannah.gnu.org/releases/attr/attr-2.5.2.tar.gz \
    || wget -q --tries=3 https://download.savannah.nongnu.org/releases/attr/attr-2.5.2.tar.gz \
    || wget -q --tries=3 https://mirror.csclub.uwaterloo.ca/nongnu/attr/attr-2.5.2.tar.gz \
    || wget -q --tries=3 https://fossies.org/linux/misc/attr-2.5.2.tar.gz) \
    && tar xf attr-2.5.2.tar.gz && cd attr-2.5.2 && ./configure --host=aarch64-linux-android --prefix=${PREFIX} --enable-static --disable-shared CC="${CC}" && make -j$(nproc) install && cp ${PREFIX}/lib/libattr.a ${LLVM}/sysroot/usr/lib/aarch64-linux-android/26/libattr.a
...
RUN printf '#ifndef PODSTEROID_SHM_SHIM_H\n#define PODSTEROID_SHM_SHIM_H\nextern int shm_open(const char *, int, unsigned);\nextern int shm_unlink(const char *);\n#endif\n' \
    > /opt/shm_shim.h
...
# Bridge
COPY podsteroid-bridge.c /tmp/podsteroid-bridge.c
RUN ${CC} --sysroot=${LLVM}/sysroot -target aarch64-linux-android26 -fPIE -pie -Wl,-z,max-page-size=16384 /tmp/podsteroid-bridge.c -o /opt/qemu-out/libpodsteroid-bridge.so

# Launcher (PR_SET_PDEATHSIG wrapper for QEMU — see podsteroid-launcher.c)
COPY podsteroid-launcher.c /tmp/podsteroid-launcher.c
RUN ${CC} --sysroot=${LLVM}/sysroot -target aarch64-linux-android26 -fPIE -pie -Wl,-z,max-page-size=16384 /tmp/podsteroid-launcher.c -o /opt/qemu-out/libpodsteroid-launcher.so
...
COPY --from=qemu-builder /opt/qemu-out/libpodsteroid-bridge.so /libpodsteroid-bridge.so
COPY --from=qemu-builder /opt/qemu-out/libpodsteroid-launcher.so /libpodsteroid-launcher.so
