...
val podsteroidQemuVersion = providers.gradleProperty("podsteroidQemuVersion").get()

android {
    namespace = "com.tiquasar.podsteroid"
    compileSdk {
        version = release(36) {
            minorApiLevel = 1
        }
    ...
    defaultConfig {
        applicationId = "com.tiquasar.podsteroid"
        minSdk = 26
        targetSdk = 36
        versionCode = 31
        versionName = "1.2.7"
        buildConfigField("String", "QEMU_VERSION", "\"$podsteroidQemuVersion\"")
    ...
    signingConfigs {
        create("release") {
            val storePath = (project.findProperty("PODSTEROID_RELEASE_STORE_FILE") as? String)
            if (storePath != null && file(storePath).exists()) {
                storeFile     = file(storePath)
                storePassword = project.findProperty("PODSTEROID_RELEASE_STORE_PASSWORD") as? String
                keyAlias      = project.findProperty("PODSTEROID_RELEASE_KEY_ALIAS")      as? String
                keyPassword   = project.findProperty("PODSTEROID_RELEASE_KEY_PASSWORD")   as? String
            }
        }
    }
...
dependencies {
    ...
    // SSH client (maintained JSch fork: rsa-sha2/ed25519, modern KEX).
