...
object AvfDiagnostics {
     ...
     * Podsteroid drives (raw kernel + initrd). A vendor build that ships AVF for
    ...
    fun runSmokeTest(context: Context): String {
        ...
        if (AvfCapabilities.choose(pre.capabilitiesRaw) is AvfCapabilities.ProtectedVmChoice.Unsupported) {
            return "not applicable on this device: the hypervisor only supports protected VMs " +
                "(caps=${pre.capabilitiesDecoded}), and Podsteroid's custom Linux kernel can run only as a " +
                "non-protected VM. This is expected, not a failure: Podsteroid automatically uses the QEMU " +
                "backend here."
        }
        ...
        return try {
            ...
            if (cause is UnsupportedOperationException &&
                ...
                "not applicable on this device: AVF rejected a non-protected VM " +
                    "(${cause.message}). Podsteroid's custom Linux kernel can run only as a non-protected " +
                    "VM. This is expected, not a failure: Podsteroid automatically uses the QEMU backend here."
            } else {
    ...
    private fun buildCustomImageConfig(kernelPath: String, initrdPath: String): Any {
        ...
        invokeSetter(builderCls, builder, "setName", String::class.java, "podsteroid-avf-smoke")
