...
class HostRequestDispatcher(
    ...
    private val onNetInfo: (HostProtocol.NetInfo) -> Unit = { _ -> },
) {
    ...
    // NETINFO <vmId> <b64json>
    private fun handleNetInfo(p: List<String>): String {
        val info = HostProtocol.parseNetInfo(p) ?: return HostProtocol.err("bad netinfo")
        onNetInfo(info)
        return HostProtocol.ok()
    }
}
