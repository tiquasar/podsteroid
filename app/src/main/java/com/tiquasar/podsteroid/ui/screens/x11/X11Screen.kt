...
fun X11Screen(
    ...
    LaunchedEffect(s.rotationLock) {
        activity?.requestedOrientation = when (s.rotationLock) {
            com.tiquasar.podsteroid.x11.RotationLock.LANDSCAPE -> ActivityInfo.SCREEN_ORIENTATION_SENSOR_LANDSCAPE
            com.tiquasar.podsteroid.x11.RotationLock.PORTRAIT  -> ActivityInfo.SCREEN_ORIENTATION_SENSOR_PORTRAIT
            com.tiquasar.podsteroid.x11.RotationLock.AUTO      -> ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED
        }
    }
    ...
    // PodsteroidService is partial/CPU-only; this is the screen-on counterpart.
    DisposableEffect(Unit) {
 ...
 * difference: an IME REPLACES composing text on commit (pinyin "nihao" ->
 * "hello"), so the new buffer shares no prefix with the old. Emitting only
