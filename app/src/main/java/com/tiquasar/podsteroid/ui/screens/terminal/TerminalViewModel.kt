 ...
 *
 * Terminal ViewModel — wires TerminalView to the podsteroid-bridge binary.
 *
 * The bridge binary (libpodsteroid-bridge.so) runs as the TerminalSession
 ...
 *     → init-podsteroid resize daemon calls stty on /dev/hvc0
...
class TerminalViewModel @Inject constructor(
    ...
    private val headlessModeManager: com.tiquasar.podsteroid.engine.hostbridge.HeadlessModeManager,
) : ViewModel() {

    /** The engine backing the currently-selected VM (null when none selected). */
    private val engine: VmEngine? get() = registry.selectedEngine.value
    ...
    val bootStage: StateFlow<String> = registry.selectedEngine
        .flatMapLatest { eng -> eng?.bootStage ?: flowOf("") }
    ...
    fun enableServerMode() {
        registry.selectedId.value?.let { headlessModeManager.setActive(it, true) }
    }
    ...
    suspend fun importThemeFromUrl(input: String): String? = withContext(Dispatchers.IO) {
        ...
        val toml = runCatching {
            val conn = (java.net.URL(tomlUrl).openConnection() as java.net.HttpURLConnection).apply {
                ...
                setRequestProperty(
                    "User-Agent",
                    "Podsteroid/${com.tiquasar.podsteroid.BuildConfig.VERSION_NAME}"
                )
            }
            try {
    ...
    fun createSession() {
        ...
        val eng = engine
        val sess = runCatching { eng?.createTerminalSession(sessionClient) }
            .onFailure { e ->
                android.util.Log.w(TAG, "createTerminalSession failed on ${eng?.backendId}: ${e.message}")
            }
            .getOrNull() ?: return
        session = sess
        attached = true
    ...
    override fun onCleared() {
        super.onCleared()
        // Drop the proxy's pointer to this dead ViewModel — otherwise the engine
        // keeps forwarding session events into a tombstoned client.
