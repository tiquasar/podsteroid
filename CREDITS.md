...
PodSteroid stands on the shoulders of an enormous amount of open-source work. The list below covers the upstream projects whose code, designs, fonts, or themes are shipped inside the APK, along with the major Android-side dependencies. Every component remains under its own upstream license; see [LICENSE](LICENSE) for PodSteroid's own license (GPL v2.0).
...
These projects make up the actual Linux system that runs inside PodSteroid.

### Linux kernel
PodSteroid ships a custom build of the upstream Linux kernel (currently 7.1.5) configured specifically for OCI-compatible containers under QEMU TCG.
...
### X11 desktop stack
The in-app X11 viewer is backed by these components running inside the VM. PodSteroid's Android side speaks RFB 3.8 directly (no third-party VNC client) and decodes the audio stream natively.
...
### QEMU
QEMU provides the machine emulation that makes everything else possible. PodSteroid currently builds against QEMU 11.0.2, cross-compiled for arm64 Android.
...
### libslirp
SLIRP is QEMU's user-mode network stack. PodSteroid links it into the QEMU binary statically.

- Project: https://gitlab.freedesktop.org/slirp/libslirp
- License: BSD-3-Clause

### libucontext
Bionic does not implement the System V `ucontext.h` API, which QEMU coroutines depend on. PodSteroid links against Ariadne Conill's portable libucontext to fill the gap.

- Project: https://github.com/kaniini/libucontext
- License: ISC

### Limbo PC Emulator
Limbo pioneered running QEMU on Android years ago and solved many of the platform-specific issues we no longer have to. Although PodSteroid is a fresh codebase with a different UI and pipeline, the lineage of "QEMU on Android" traces back to Max Kastanas's work.
...
The terminal layer (xterm-256color emulator, escape-sequence parsing, mouse tracking, color theme parsing) is the Termux project's `terminal-view` and `terminal-emulator` libraries, consumed via JitPack as `com.github.termux:terminal-view:0.118.1`. PodSteroid rebuilds `libtermux.so` locally for 16 KB page alignment.
...
PodSteroid itself is developed by **[ExTV](https://github.com/ExTV)** and contributors.

- Repository: 
- Website: https://extv.github.io/PodSteroid/
...
We take attribution seriously. If you believe your work is shipped in PodSteroid but not properly credited, or if any information above is incorrect, please [open an issue](/new) and we will get it fixed quickly.
