 ...
 *
 * podsteroid-vsock-agent — guest-side peer for PodSteroid's AVF backend.
 *
 * Reads /etc/podsteroid/forwards.conf at startup. Each line is one of:
 *
 ...
 *                                  to /run/term_size for podsteroid-login restore
 ...
 *
 * Invoked as `podsteroid-vsock-agent downloads-9p` (argv[1] dispatch, separate
...
#define CONFIG_PATH     "/etc/podsteroid/forwards.conf"
#define TERM_SIZE_PATH  "/run/term_size"
#define LOG_TAG         "podsteroid-vsock-agent"
...
#define PODSTEROID_DOWNLOADS_VSOCK_PORT 200000
#define PODSTEROID_DOWNLOADS_MOUNT      "/mnt/downloads"
 ...
 * (we saw "podsteroid-vsock-agent [error] podsteroid-vsock-agent [error] ctl
...
static void handle_resize(int rows, int cols) {
     ...
     * ~14 KB/s. The getty also moved to hvc0 (per podsteroid-getty + the
     * `podsteroid.tty=hvc0` cmdline marker AVF now passes). */
...
static void downloads_9p_serve(void) {
    ...
    sa.svm_family = AF_VSOCK;
    sa.svm_cid    = VMADDR_CID_ANY;
    sa.svm_port   = PODSTEROID_DOWNLOADS_VSOCK_PORT;
    if (bind(s, (struct sockaddr *)&sa, sizeof(sa)) < 0) {
        LOG_E("downloads-9p: vsock bind(%d) failed: %s", PODSTEROID_DOWNLOADS_VSOCK_PORT, strerror(errno));
        close(s);
        return;
    }
    if (listen(s, 1) < 0) {
        LOG_E("downloads-9p: vsock listen(%d) failed: %s", PODSTEROID_DOWNLOADS_VSOCK_PORT, strerror(errno));
        close(s);
        return;
    }
    LOG_I("downloads-9p: listening on vsock:%d", PODSTEROID_DOWNLOADS_VSOCK_PORT);
    ...
    for (;;) {
        ...
        if (umount2(PODSTEROID_DOWNLOADS_MOUNT, MNT_DETACH) < 0 && errno != EINVAL && errno != ENOENT)
