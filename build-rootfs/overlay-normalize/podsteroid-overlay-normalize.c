 ...
 *
 * podsteroid-overlay-normalize <upper_dir> <work_dir>
 *
 ...
 * PODSTEROID_NORMALIZE_NS to use "user.overlay." (no CAP_SYS_ADMIN needed).
 *
...
int main(int argc, char **argv) {
    if (argc != 3) {
        fprintf(stderr, "usage: podsteroid-overlay-normalize <upper_dir> <work_dir>\n");
        return 2;
    }
    const char *ns = getenv("PODSTEROID_NORMALIZE_NS");
