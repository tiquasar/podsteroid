...
public final class TerminalRenderer {
    ...
    // PodSteroid: hoisted out of the per-cell hot path. Image-heavy frames (chafa,
    // gnuplot) draw one Sixel bitmap per cell — allocating a fresh RectF each
    // time was straight-up GC pressure with no benefit since we just mutate it.
    private final RectF mBitmapDestRect = new RectF();

    // PodSteroid: cache last paint color so repeated runs with the same color skip
    ...
    public TerminalRenderer(int textSize, Typeface typeface) {
        ...
        // Programming-font ligatures (FiraCode, JetBrains Mono, Cascadia Code).
        // `liga` = standard ligatures, `calt` = contextual alternates — most
        // programming fonts ship their `=>`, `===`, `!=`, etc. under `calt`.
        // Android's text engine handles the shaping inside drawTextRun; the
        // run-coalescing logic above already groups same-style cells, so the
        // ligature glyph paints naturally over its source cells.
        mTextPaint.setFontFeatureSettings("'liga' on, 'calt' on");
    ...
    public final void render(TerminalEmulator mEmulator, Canvas canvas, int topRow,
        ...
        // PodSteroid: reset Paint attribute state at the start of every frame so
            ...
            for (int column = 0; column < columns; ) {
                ...
                if (TextStyle.isTerminalBitmap(style)) {
                    ...
                    if (bitmap != null) {
                        ...
                        mBitmapDestRect.set(left, top, left + mFontWidth, top + mFontLineSpacing);
                        canvas.drawBitmap(bitmap, bitmapSrcRect, mBitmapDestRect, null);
                    }
                    column += 1;
    ...
    private void drawTextRun(Canvas canvas, char[] text, int[] palette, float y, int startColumn, int runWidthColumns,
        ...
        if ((effect & TextStyle.CHARACTER_ATTRIBUTE_INVISIBLE) == 0) {
            ...
            // PodSteroid: always set all four attributes per run — they MUST be
